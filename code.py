...
# Chargement du fichier CSV dans un DataFrame avec le bon séparateur
cities_data = pd.read_csv('medium.csv', header=None)

# Debug : Affiche les premières lignes et colonnes pour vérifier
...
# Extraction des coordonnées des villes (Latitude et Longitude)
...
BETA = 2  # Influence de la visibilité (1/distance)
EVAPORATION = 0.5  # Taux d'évaporation des phéromones
Q = 100  # Quantité de phéromones déposées
INIT_PHEROMONE = 0.1  # Valeur initiale des phéromones
...
distances = np.array([[euclidean_distance(a, b) for b in cities] for a in cities])
np.fill_diagonal(distances, np.inf)  # Pas de boucle sur soi-même
...
for iteration in range(N_ITER):
    ...
    for ant in range(N_ANTS):
        # Chaque fourmi commence dans une ville aléatoire
        path = [np.random.randint(0, N_CITIES)]
        while len(path) < N_CITIES:
            current = path[-1]
            unvisited = list(set(range(N_CITIES)) - set(path))
...
# ==============================
# 📌 VISUALISATION DU MEILLEUR CHEMIN
# ==============================
...
plt.title(f"Meilleur chemin trouvé (Longueur : {best_path_length:.2f})")
